    ...
    def singleLinkageDist(self, other):
        ...
        for i in self.points:
            for j in other.points:
                distances.append(i.distance(j)) #distance method of class Point
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        new_points=c1.points[:]  #otherwise the original list with be mutated
        for i in c2.points:
            if not c1.isIn(i.getName()):
                new_points.append(i)
        new_cluster=Cluster(new_points, c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        minDist=float("inf")
        for i in range (len(clusters)-1):  #possible alternative implementation with itertools
            for j in range (i+1, len(clusters)):
                distancia=linkage(clusters[i], clusters[j])
                if distancia < minDist:
                    minDist=distancia
                    best_tuple=(clusters[i], clusters [j])
        return best_tuple
